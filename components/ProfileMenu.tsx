  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="max-w-28 truncate">{session?.user?.name || "Perfil"}</span>
        <span>▾</span>
      </button>
          ...
          {status !== "authenticated" ? (
            <div>
              ...
              <button
                ...
                className="mt-5 w-full rounded-xl bg-[#5865f2] px-4 py-3 font-black text-white"
              >
                ◉ Entrar con Discord
              </button>
            ...
            <div>
              <div className="flex items-center gap-3 border-b border-white/10 pb-4">
                {session.user.image && (
                  <Image
                    ...
                    width={54}
                    height={54}
                    className="rounded-full border border-[#c084fc]/50"
                  />
                )}
                <div className="min-w-0">
                  <p className="truncate font-black">{session.user.name}</p>
                  <span className="mt-1 inline-flex rounded-full bg-[#4c1d95] px-2.5 py-1 text-[11px] font-black text-[#e9d5ff]">
                    {active ? "★ Premium Member" : "No Member"}
                  </span>
                </div>
              </div>
              <button
                ...
                className="mt-3 flex w-full items-center justify-between rounded-xl px-3 py-3 text-left hover:bg-white/[.06]"
              >
                <span>♙ Mi Perfil</span>
                <span>›</span>
              </button>
              ...
              <button
                ...
                className="mt-3 w-full border-t border-white/10 px-3 pt-4 text-left font-bold text-[#fb7185]"
              >
                ↪ Cerrar sesión
              </button>
            </div>
